...
class ProfessorPorTurmaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $professor = $this->user->find($request->professor);
        $idTurmaOficiais = 7;

        if ($professor && $professor->perfil_id === 3 && $request->turma != $idTurmaOficiais) {
            $jaAssociado = $professorPorTurma->where([
                'professor_id' => $request->professor,
                'turma_id'     => $idTurmaOficiais,
            ])->exists();
